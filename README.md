...
> Keywords / Topics: i-manju i-drama comfyui grok-bot minimax-h3 wan21 fmpeg storyboard character-bible local-ai-video chinese-donghua open-source

**当前版本：** [0.0.1](https://github.com/wicm84266964/grokbot-ai-manju/releases/tag/v0.0.1)
...
| 能力 | 说明 |
|------|------|
| 整本小说入库 | 建世界观/势力/地点/伏笔表 + 分集地图 pisode_map |
...
## 全链路（Skill i-manju-pipeline）

1. **系列层**：整本小说 → ible/ + plan/episode_map.yaml
2. **人物层**：参考图 → 三视图 + oice/profile.yaml（可选 sample.wav）
...
- 本机 ffmpeg / wheels 放本仓库 	ools/（gitignore）
- 模型权重不进 Git
...
**有：** README、CHANGELOG.md、prompts/、skills/、docs/、	emplates/、workflows/ 模板、scripts/、src/ CLI 脚手架、MIT License

**默认没有（gitignore）：** shows/ 剧集媒体、	ools/ 二进制、模型、venv、密钥
...
- 开源脚手架与目录契约冻结
- Skill i-manju-pipeline：整本优先 + 质检/返工/发行设计
